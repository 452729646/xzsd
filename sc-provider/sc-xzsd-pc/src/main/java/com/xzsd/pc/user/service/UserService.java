...
public class UserService {
    ...
    public AppResponse updateUser(UserInfo userInfo) {
        if (userInfo.getRole().equals(userInfo.getNowRole())){
            return AppResponse.bizError("同等级角色无法进行修改");
        }
    ...
    public AppResponse deleteUser(UserSettingDTO userSettingDTO) {
        userSettingDTO.setUserList(Arrays.asList(userSettingDTO.getUserId().split(",")));
        List<String> listRole = Arrays.asList(userSettingDTO.getRole().split(","));
        for (String role : listRole){
            if (userSettingDTO.getNowRole().equals(role)){
                return AppResponse.bizError("同等级角色无法进行删除");
            }
            if ("2".equals(userSettingDTO.getNowRole())){
