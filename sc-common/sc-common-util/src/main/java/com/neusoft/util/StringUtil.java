...
label0:
     ...
     * 作成时间：2017-10-11
     */
    public static String getSkuNo(int num){
        String prefix = new SimpleDateFormat("MMddHHmm").format(new Date());
        String suffix = RandomUtil.radmonkey(num);
        return prefix + suffix;
    }
    /**
     ...
     * 作成时间：2017-10-11
     */
    public static String getBannerCode(int num){
        String prefix = new SimpleDateFormat("HHmm").format(new Date());
        String suffix = RandomUtil.radmonkey(num);
        return prefix + suffix;
    }
    /**
     ...
     * 作成时间：2020-3-25
     */
    public static String getCateCode() {
        String cateCode = "";
        // 用字符数组的方式随机
        ...
        for (int j = 0; j < 6; j++) {
            ...
            // 保证六位随机数之间没有重复的
            if (cateCode.contains(String.valueOf(c))) {
                j--;
                continue;
            }
            cateCode = cateCode + c;
     ...
     * 作成时间：2020-3-25
     */
    public static String getHotGoodsCode() {
        String hotGoodsCode = "";
        // 用字符数组的方式随机
        String model = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        char[] m = model.toCharArray();
        for (int j = 0; j < 6; j++) {
            char c = m[(int) (Math.random() * 36)];
            // 保证八位随机数之间没有重复的
            if (hotGoodsCode.contains(String.valueOf(c))) {
                j--;
                continue;
            }
            hotGoodsCode = hotGoodsCode + c;
        }
        return hotGoodsCode;
    }
     ...
     * 作成时间：2017-10-11
     */
    public static String getDriverNo(int num){
        String prefix = new SimpleDateFormat("mmssSSS").format(new Date());
        String suffix = RandomUtil.radmonkey(num);
        return prefix + suffix;
    }
